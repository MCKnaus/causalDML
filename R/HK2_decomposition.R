...
#'          c("DiM","ADiM","d0","s1","s2","s3","d1","d2","d3","Cov(etX,mut|Xg)","d4","SRCT2","d4'","d5").}
...
HK2_decomposition = function(Y,A,G,
  ...
  dimnames(TP_level)[[1]] = dimnames(TP_level_IF)[[1]] = c("DiM","ADiM","d0","s1","s2","s3","d1","d2","d3",
...
HK2_parameter_maker = function(level,
  ...
  for (p in 1:14) {
    if (length(t_aggregate) == 1 & length(x_aggregate) == 1) {
      ...
      rownames(results) = c("DiM","ADiM","d0","s1","s2","s3","d1","d2","d3",
                            "Cov(etX,mut|Xg)","d4","SRCT2","d4'","d5")
    }
    ...
    if (length(t_aggregate) == 1 & length(x_aggregate) == 2) {
      ...
      rownames(results) = c("DiM","ADiM","d0","l1","l2","l3","d1","d2","d3",
                            "Cov(etX,mut|Xg)","d4","SRCT2","d4'","d5")
    }
